...
static int io_init(struct ubi_device *ubi)
{
    ...
    ubi->save_scan_info_advance = 1;
  
    if (ubi->save_scan_info_advance) {
        int err;
        
        ubi->peb_count -= 1;
        ubi->scan_info_block = ubi->peb_count;
        
        err = ubi->mtd->block_isbad(ubi->mtd, (loff_t)ubi->scan_info_block * ubi->peb_size);
        if (err < 0)
            ubi_err("error %d while checking if PEB %d is bad", err, ubi->scan_info_block);
        else if (err){
            ubi_msg("scan info block is bad, PEB[%d]", ubi->scan_info_block); 
            ubi->save_scan_info_advance = 0;        
        }        
    }
    
	ubi_msg("save scan info advance:     %d", ubi->save_scan_info_advance);
#endif
