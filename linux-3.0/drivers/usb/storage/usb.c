...
void usb_stor_disconnect(struct usb_interface *intf)
{
	...
	/*201120720: SCSI cmd's time_out is too long(SD_TIMEOUT 30s), 
	so need stop scatter/gather i/o issued by usb_sg_wait() manually*/
	if(us)
	{
		if(test_and_clear_bit(US_FLIDX_SG_ACTIVE, &us->dflags))
		{
			printk("usb-storage: cancelling sg request\n");
			usb_sg_cancel(&us->current_sg);
		}
		if(test_and_clear_bit(US_FLIDX_URB_ACTIVE, &us->dflags))
		{
			printk("usb-storage: cancelling URB\n");
