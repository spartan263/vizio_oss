...
const UINT32 _u4MsdcFreq1[] = {MSDC_CLOCK_SELECTION_STRING};
const UINT32 _u4MsdcFreqSelVal1[] = {MSDC_CLOCK_SELECTION_VALUE};
UINT32 _u4MsdcFreqIdx1 = SD_DEFAULT_NORM_CLOCK_INDEX;
UINT32 _u4MsdcCurFreq1 = 48;
...
const EMMC_FLASH_DEV_T _arEMMC_DevInfo1[] =
{
    ...
    { "H26M31003GMR",       0x4A483447, 0x31640402, 0x00000000},
...
void MsdcDumpRegister(void)
{
    ...
    for(; i< 0x104; i+=4)
    {
        ...
        MSDC_LOG(MSG_LVL_ERR, " %08X", MSDC_READ32(MSDC_CFG(ch1) + i));
    }
}
...
void MsdcSetSample1(UINT8 fgFalling)
{
    // Set command response sample selection
    if(fgFalling & 0x0F)
    {
        MSDC_SETBIT(MSDC_IOCON(ch1), ((0x1) << 1)); 
    } 
    else
    {
        MSDC_CLRBIT(MSDC_IOCON(ch1), ((0x1) << 1));
    }
	    
    // Set read data sample selection
    if((fgFalling>>4) & 0x0F)
    {
        MSDC_SETBIT(MSDC_IOCON(ch1), ((0x1) << 2));  
    }
    else
    {
        MSDC_CLRBIT(MSDC_IOCON(ch1), ((0x1) << 2));
    }		
}
...
void MsdcSetSampleEdge1(UINT32 fgWhere)
{
    UINT8 u1Sample;

    u1Sample = (_arEMMC_DevInfo1[msdc_host1->devIndex].u4Sample>>(8*fgWhere)) & 0xFF;

    MSDC_LOG(MSG_LVL_INFO, "[1]Dev Num: %d, Timing Position: %d, Sample Edge: 0x%02X\n", msdc_host1->devIndex, fgWhere, u1Sample);

    // Set Sample Edge
    MsdcSetSample1(u1Sample);
...
INT32 MsdcSetClkfreq1(UINT32 clkFreq)
{
    ...
    index = sizeof(_u4MsdcFreq1)/sizeof(UINT32) - 1;
    if(clkFreq >= _u4MsdcFreq1[0])
    {
        sdClkSel = _u4MsdcFreqSelVal1[0];
        expFreq = _u4MsdcFreq1[0];    	
    }
    else if(clkFreq <= _u4MsdcFreq1[index])
    {
        sdClkSel = _u4MsdcFreqSelVal1[index];
        expFreq = _u4MsdcFreq1[index]; 	
    }
    else
    {
        for(index -= 1;index > 0;index--)
        {
            if((clkFreq >= _u4MsdcFreq1[index]) && (clkFreq < _u4MsdcFreq1[index-1]))
            {
                sdClkSel = _u4MsdcFreqSelVal1[index];
                expFreq = _u4MsdcFreq1[index]; 
                break;	
    ...
    if (_u4MsdcCurFreq1 == expFreq)
    {
        return MSDC_SUCCESS;
    }

    // Gate msdc_src_clk
    MSDC_SETBIT(MSDC_SCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);
    ...
    MSDC_CLRBIT(MSDC_SCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_SLECTION_MASK);  
    MSDC_SETBIT(MSDC_SCLOCK_SELECTION_REG1, sdClkSel<<0);  

    // Disable gating msdc_src_clk
    MSDC_CLRBIT(MSDC_SCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);    
    ...
    MSDC_LOG(MSG_LVL_INFO, "[1]Set Bus Clock as %d(MHz) Success!\n", expFreq);	
    _u4MsdcCurFreq1 = expFreq;
...
int MsdcReqCmdTune1(struct msdc_host *host, struct mmc_command *cmd, struct mmc_data *data)
{
    ...
    MSDC_LOG(MSG_LVL_ERR, "[1]----->Go into Command Tune!\n");
...
INT32 MsdcErrorHandling1(struct msdc_host *host, struct mmc_command *cmd, struct mmc_data *data)
{
    ...
    MSDC_LOG(MSG_LVL_INFO, "[1]End Error Handling...!\n");
...
INT32 MsdcReqDataBasicDMA1(struct msdc_host *host, struct mmc_data *data)
{
    ...
    while(u4BufLen)
    {
        ...
        if((host->num_sg == 1) && ((host->remain) <= BASIC_DMA_MAX_LEN))
        {
            ...
            msdc_host1->desVect = IntrWaitVect; //avoid isr/thread context race condition
            msdc_host1->waitIntMode = 1; 
        ...
        else
        {
            ...
            msdc_host1->desVect = INT_DMA_XFER_DONE; //avoid isr/thread context race condition
            msdc_host1->waitIntMode = 1; 
...
int MsdcReqDataReadTune1(struct msdc_host *host, struct mmc_command *cmd, struct mmc_data *data)
{
    ...
    do 
    {
        for (dsmpl = 0; dsmpl < 2; dsmpl++) 
        {
            ...
            MSDC_LOG(MSG_LVL_INFO, "[1]TUNE_READ<%s> dcrc<0x%x> DATRDDLY0/1<0x%x><0x%x> dsmpl<0x%x> cmd_error<%d> data_error<%d>",
    ...
    do 
    {
        ...
        {    
            for (dsmpl = 0; dsmpl < 2; dsmpl++) 
            {
                ...
                MSDC_LOG(MSG_LVL_ERR, "[1]TUNE_WRITE<%s> DSPL<%d> DATWRDLY<%d> MSDC_DAT_RDDLY0<0x%x> cmd_error<%d> data_error<%d>", 
 ...
 */
static void __devinit msdc1_request_dma(struct msdc_host *host)
{
...
static void __devinit msdc1_release_dma(struct msdc_host *host)
{
    /* using dma_alloc_coherent*/  
 ...
 * Allocate/free IRQ.
 */
static int __devinit msdc1_request_irq(struct msdc_host *host)
{
...
static int msdc1_platform_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    //gate hclk & sclk
    MSDC_SETBIT(MSDC_HCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);	
    MSDC_SETBIT(MSDC_SCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);	
...
static int msdc1_platform_resume(struct platform_device *dev)
{
    ...
    //gate hclk & sclk
    MSDC_CLRBIT(MSDC_HCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);	
    MSDC_CLRBIT(MSDC_SCLOCK_SELECTION_REG1, MSDC_CLOCK_CKGEN_GATE_BIT);	
...
static int __devinit msdc1_probe(struct platform_device *pdev)
{
 ...
 */
static void __devexit msdc1_shutdown(struct device *dev)
{
...
static int __devexit msdc1_remove(struct platform_device *dev)
{
    MSDC_LOG(MSG_LVL_INFO, "\n");
...
static struct platform_driver msdc1_driver = {
    .probe    = msdc1_probe,
    .remove   = __devexit_p(msdc1_remove),
