                   ...
                   bool       *ret_none )
{
     ...
     if(single && ((single->dst.w != single->src.w) || (single->dst.h != single->src.h)))
     {
          return NULL;
     }
     ...
     *config = *current; /* make sure that all fields in "request" are valid */
     ...
     if (flags & CWCF_STACKING)
     {
...
#ifdef CC_HW_WINDOW_SUPPORT
          {
               ...
               direct_list_foreach (tier, sawman->tiers)
               {
                    if(tier->classes == (1 << config->stacking))
                    {
                         break;
                    }
               }
               ...
               if(NULL != tier)
               {
                    ...
                    if(none || single)
                    {
                          config->bounds.w = sawwin->window->surface->config.size.w;
                          config->bounds.h = sawwin->window->surface->config.size.h;
                    }
                    else
                    {
                          DFBDisplayLayerConfig layer_config;
                          dfb_layer_context_get_configuration(tier->context, &layer_config);
                          if(layer_config.buffermode != DLBM_WINDOWS)
                          {
                              config->bounds.w = layer_config.width;
                              config->bounds.h = layer_config.height;
                          }
                          else
                          {
                              config->bounds.w = sawwin->window->surface->config.size.w;
               ...
               D_INFO("reconfig window size [%d, %d] \n", config->bounds.w, config->bounds.h);
          }
#endif          
     }   
