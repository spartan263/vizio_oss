...
#ifndef CC_DFB_SHARE_MEMORY	 
     if(DFB_SYSTEMONLY&&(!DSCAPS_FROM_VIDEO(surface_caps)))
     {
        surface_caps = surface_caps&(~DSCAPS_VIDEOONLY);
        scon.caps   = surface_caps | DSCAPS_SYSTEMONLY;
     }
     else
     {
        surface_caps = surface_caps&(~DSCAPS_SYSTEMONLY);
        scon.caps   = surface_caps | DSCAPS_VIDEOONLY;
     }
#endif
...
dfb_window_create_region(CoreWindow *window)
{
    ...
    config.options       = context->config.options & DLOP_FLICKER_FILTERING;
    config.source        = (DFBRectangle) { 0, 0, config.width, config.height };
    config.dest          = (DFBRectangle) { 0, 0, config.width, config.height };
                   ...
                   CoreWindow                 **ret_window )
{
     ...
     colorspace   = desc->colorspace;
			   ...
			   surface_caps = desc->surface_caps & (DSCAPS_INTERLACED	 | DSCAPS_SEPARATED  |
													...
													DSCAPS_VIDEOONLY | DSCAPS_FROM_3DMM|DSCAPS_FOUCE_VIDEOONLY);
		  /*
		  ...
		  */	 
		  ...
		  surface_caps = desc->surface_caps & (DSCAPS_INTERLACED	| DSCAPS_SEPARATED	|
											   DSCAPS_PREMULTIPLIED | DSCAPS_DEPTH		|
											   DSCAPS_STATIC_ALLOC	| DSCAPS_SYSTEMONLY |
											   DSCAPS_VIDEOONLY|DSCAPS_FOUCE_VIDEOONLY);
#endif
     ...
     if (colorspace == DSCS_UNKNOWN) {
		  ...
		  if(context->config.buffermode == DLBM_WINDOWS)
		  {
			  ...
			  surface_caps |= DSCAPS_VIDEOONLY | DSCAPS_FOUCE_VIDEOONLY;
		  }
		  else
#endif
	 ...
	 if(DFB_SYSTEMONLY)
#else	 	
	 if((surface_caps & DSCAPS_SYSTEMONLY)&&(DFB_SYSTEMONLY))
#endif	 	
   	 {
