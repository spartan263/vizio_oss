                             ...
                             unsigned int            *ret_num )
{
     ...
     /*
     direct_log_printf(NULL,"%s[0x%x,0x%x,0x%x,0x%x,0x%x] \n",__FUNCTION__,
        surface->type,
        buffer->policy,
                            ...
                            CoreSurfaceAllocation  **ret_allocation )
{
    ...
    if(DFB_SYSTEMONLY)
    {
        ...
        {
            buffer->policy = CSP_VIDEOONLY;
            buffer->surface->config.caps &= (~DSCAPS_SYSTEMONLY);
            buffer->surface->config.caps |= (DSCAPS_VIDEOONLY);
        }
    }
#endif    
    /*
    direct_log_printf(NULL,"%s[0x%x,0x%x,0x%x] \n",__FUNCTION__,surface->type,buffer->policy,buffer->surface->config.caps);
    */
