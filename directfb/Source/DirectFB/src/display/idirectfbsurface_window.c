...
static DFBDisplayLayerID 
layerid_by_stacking(DFBWindowStackingClass   stacking)
{
    int i = 0;
	...
	{
         config_layer = &dfb_config->layers[i];

         if(config_layer->init && (config_layer->stacking & (1 << stacking)))
		 {
		      D_DEBUG_AT( Surface, "stacking [0x%x], layer_id [%d] \n", stacking, i);
		      return (DFBDisplayLayerID) i;
		 }
	}

	D_DEBUG_AT( Surface, "stacking [0x%x] can not find layer_id \n", stacking);

	return 0;//default 0 FIXME
}
                              ...
                              DFBSurfaceFlipFlags  flags )
{
	 ...
	 DFBDisplayLayerID on_layerid = layerid_by_stacking(data->window->config.stacking);
     D_DEBUG_AT( Surface, "on_layerid [0x%x], layer_id [%d] \n", on_layerid, data->window->stack->context->layer_id);  
     if ((on_layerid == data->window->stack->context->layer_id) 
	 	&&data->window->region && (data->window->region->context->config.buffermode == DLBM_WINDOWS))
#else
...
#ifdef CC_HW_WINDOW_SUPPORT	  
		  if((on_layerid != data->window->stack->context->layer_id) 
		  	|| (data->window->stack->context->config.buffermode != DLBM_WINDOWS))
		  {
