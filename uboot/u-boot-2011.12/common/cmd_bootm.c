...
#if (defined(CONFIG_NAND_BOOT) || defined(CC_EMMC_BOOT)) && defined(CONFIG_CMD_JFFS2) && defined(CONFIG_JFFS2_CMDLINE)
int do_mtd_update(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    sprintf(p1, "0x%lx", dramaddr);
    local_args[3] = p1;
    
    sprintf(p2, "0x%lx", part_offset);
    local_args[4] = p2;
    
    sprintf(p3, "0x%lx", filesize);
    local_args[5] = p3;

    if (do_mmcops(cmdtp, 0, 6, local_args))
    {
        return -1;
    }
#else
