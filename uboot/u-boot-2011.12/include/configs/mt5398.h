...
#if defined(CC_EMMC_BOOT)
    #define CONFIG_BOOTARGS_BASE    "root=/dev/mmcblk0p7 rootwait rootfstype=squashfs ro console=ttyMT0,115200n1 vmalloc=450mb"
    #define CONFIG_BOOTARGS_BASE2   "root=/dev/mmcblk0p8 rootwait rootfstype=squashfs ro console=ttyMT0,115200n1 vmalloc=450mb"
    #if CONFIG_ANDROID_BOOT
	    #define CONFIG_BOOTARGS_BASE_ANDROID  "lpj=4571136 console=ttyMT0,115200n1 root=/dev/mmcblk0p5 vmalloc=450M"
    #else
        #define CONFIG_BOOTARGS_BASE_ANDROID  "lpj=4571136 console=ttyMT0,115200n1 ubi.mtd=6 root=ubi0:rootfs" \
                                              " rootfstype=ubifs init=/init vmalloc=450MB"
    #endif //CONFIG_ANDROID_BOOT
    
#elif !defined(CC_KERNEL_ON_NOR)
    #define CONFIG_BOOTARGS_BASE    "root=/dev/mtdblock6 rootwait rootfstype=squashfs ro console=ttyMT0,115200n1 vmalloc=450mb"
    #define CONFIG_BOOTARGS_BASE2   "root=/dev/mtdblock7 rootwait rootfstype=squashfs ro console=ttyMT0,115200n1 vmalloc=450mb"
    #if CONFIG_ANDROID_BOOT
        #define CONFIG_BOOTARGS_BASE_ANDROID  "console=ttyMT0,115200n1 root=/dev/ram vmalloc=450mb"
    #else
    #define CONFIG_BOOTARGS_BASE_ANDROID  "console=ttyMT0,115200n1 ubi.mtd=6 root=ubi0:rootfs" \
                                              " rootfstype=ubifs init=/init vmalloc=450mb"
    #endif //CONFIG_ANDROID_BOOT
#else
    #define CONFIG_BOOTARGS_BASE    "root=/dev/ram console=ttyMT0,115200n1 vmalloc=450mb"
#endif
...
#define PHYS_SDRAM_2_SIZE   (DEFAULT_CHANNEL_B_SIZE-0x7800000)//((IS_DRAM_SIZE_MORE_THAN_1G) ? (TOTAL_MEM_SIZE/4-0x7800000) : (TOTAL_MEM_SIZE-0x7800000)) 
#else
