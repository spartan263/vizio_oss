...
export PRELOADER_BIN_DIR := $(shell mkdir -p $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/preloader && cd $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/preloader && /bin/pwd)
export LOADER_BIN_DIR    := $(shell mkdir -p $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/loader && cd $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/loader && /bin/pwd)
export MTKLOADER_BIN_DIR := $(shell mkdir -p $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/mtkloader && cd $(MTKLOADER_OBJ_ROOT)/project_x/target/$(TARGET_IC)/mtkloader && /bin/pwd)


all: do_drv_inc
	$(MAKE) $(if $(filter -j,$(MAKEFLAGS)),,-j $(JOBS)) -C $(PROJECT_ROOT)/target/$(TARGET_IC)/mtkloader TARGET=$(TARGET_IC) LINUX_SOLUTION=false UBOOT_LIBRARY= --no-print-directory -s | tee  $(THIS_ROOT)/build.mtkloader.log
