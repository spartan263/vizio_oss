...
INT32 BIM_CheckMisc(void)
{
    ...
    emmc_read_from_partition((unsigned char*)pmisc_msg, "misc", 2048, sizeof(misc_message));
#else
    extern int nand_read_from_partition(unsigned char* buf, char* pname, unsigned int offset, unsigned int size);
    nand_read_from_partition((unsigned char*)pmisc_msg, "misc", 2048, sizeof(misc_message));
#endif
...
{
    // Check T8032 SRAM. Must be first to clear SRAM if set.
    extern INT32 BIM_CheckT8032Recovery(void);
    if (BIM_CheckT8032Recovery())
        return 1;
